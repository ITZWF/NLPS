...
from utils import bert_tokenizer
...
class BertCrfParallel(nn.Module):

    def __init__(self, tag_to_ix, cuda_flag=False):
        super(BertCrfParallel, self).__init__()
    ...
    def bert_emb(self, inputs):
        # inputs_emb = torch.zeros(size=[inputs.size(0), inputs.size(1), 768])
        # if self.cuda:
        #     inputs_emb = inputs_emb.cuda()
        self.bert.eval()
        with torch.no_grad():
            # 在这增加padding mask
            input_mask = inputs != bert_tokenizer.pad_token_id
            input_mask = input_mask.float()
            bert_layers = self.bert(inputs, input_mask)
            # last_4_layers = bert_layers[-4:]
            # for layer in last_4_layers:
            #     inputs_emb = torch.add(layer, inputs_emb)
            inputs_emb = bert_layers.last_hidden_state
    ...
    def _get_lstm_features_parallel(self, sentence):
        # shape  sentence [bs, seq_len]
        # self.hidden = self.init_hidden()
        # embeds = self.word_embeds(sentence)  # [bs, seq_len, emb_dim]
        embeds = self.bert_emb(sentence)
        # lstm_out, self.hidden = self.lstm(embeds)  # lstm_out [bs, seq_len, hidden_dim * 2]
        # self.hidden [bs, biLSTM, hidden_dim]
        # lstm_feats = self.hidden2tag(lstm_out)
        lstm_feats = self.hidden2tag(embeds)
        # lstm_feats [bs, seq_len, num_tags]
        return lstm_feats
    ...
    def _score_sentence_parallel(self, feats, tags):
        ...
        # shape  feats [bs, seq_len, num_tags]  tags [bs, num_tags]
        ...
        for i in range(feats.shape[1]):
            feat = feats[:, i, :]
            # feat [bs, num_tags]
            trs_score = self.transitions[tags[:, i + 1], tags[:, i]]
            # [bs]
            ems_score = feat[range(feat.shape[0]), tags[:, i + 1]]
            # [bs]
            score = score + trs_score + ems_score
        score = score + self.transitions[self.tag_to_ix[STOP_TAG], tags[:, -1]]
        # score [bs]
        return score
    ...
    def neg_log_likelihood_parallel(self, sentences, tags):
        # shape: sentence [bs, seq_len], tags [bs, seq_len]
    ...
    def forward(self, sentence):  # dont confuse this with _forward_alg above.
        ...
        # 这只允许batch=1的情况
        lstm_feats = lstm_feats.view([lstm_feats.size(1), lstm_feats.size(2)])
...
if __name__ == '__main__':
    ...
    model = BertCrfParallel(tag_to_ix, cuda_flag=False)
